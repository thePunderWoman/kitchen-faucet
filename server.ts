...
import {KitchenFaucetRootServerModule} from './src/main.server';

// The Express app is exported so that it can be used by serverless Functions.
export function app(): express.Express {
  const server = express();
  const distFolder = join(process.cwd(), 'dist/kitchen-faucet/browser');
  ...
  server.engine('html', ngExpressEngine({
                  bootstrap: KitchenFaucetRootServerModule,
                }));
...
// The below code is to ensure that the server is run only when not requiring
// the bundle.
